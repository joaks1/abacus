...
option_list <- list(
    ...
    make_option(c("-o", "--output_path"), type="character",
        dest="output_path", default="posterior_table.txt",
...
#        p.tbl <- merge.2tbl.byName(transformed.posterior.p.tbl[1,], this.prior.p)
	post.tbl = transformed.posterior.p.tbl[1,]
	prior.tbl = post.tbl
        for (i in 1:length(prior.tbl)) {
            prior.tbl[i] = 1/nPairs
        }
	p.tbl <- merge.2tbl.byName(post.tbl, prior.tbl)
      ...
      prior.tbl = raw.posterior.p
      for (i in 1:length(prior.tbl)) {
          prior.tbl[i] = 1/nPairs
      }
      p.tbl <- merge.2tbl.byName(raw.posterior.p, prior.tbl)
      rownames(p.tbl) <- c("posterior.p", "prior.p")
...
res = stdAnalysis(obs.infile=options$observed_path,
		  ...
		  posterior.tbl.file=options$output_path)
